...
export default function PanelWhatsApp({ showToast }: { showToast: (text: string, ok?: boolean) => void }) {
  ...
  const [quickReplies, setQuickReplies] = useState<{ id: string; texto: string; color?: string; producto?: string }[]>([]);
  ...
  const startEditQuickReply = (qr: { id: string; texto: string; color?: string; producto?: string }) => {
          ...
          {!selected ? (
            ...
            <>
              <div className="px-6 py-4 border-b border-[#F0F2F5] flex items-center justify-between gap-3 flex-wrap">
                ...
                <div className="flex items-center gap-2">
                  <a href={`tel:+${selected.phone}`} title="Llamar (número normal, no es llamada de WhatsApp)"
                  ...
                  <button
                    onClick={() => { navigator.clipboard.writeText(selected.phone); showToast('Número copiado'); }}
                    title="Copiar número"
                    className="w-8 h-8 flex-shrink-0 rounded-[8px] border-none cursor-pointer flex items-center justify-center text-[#5A6270] bg-[#F4F6F8] hover:bg-[#ECEEF2] transition">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" /><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1" /></svg>
                  </button>
